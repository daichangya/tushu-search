...
$(document).ready(function () {
        ...
        if ($(this).hasClass('add-color')) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.status == 1) {
                        $('#tushu_book').removeClass('add-color');
                    }
                    if (data.status == -1) {
                        alert('您还没有登录');
                    }
    ...
    $('#bookMark').click(function () {
        ...
        bookmarkurl = "/tushu_content?url=" + url + "&name=" + content_name + "&chapter_url=" + chapter_url + "&novels_name=" + novels_name;
    ...
    // login
    $("#tushu_login").click(function () {
        var tushu_user = $("#tushu_user").val();
        var tushu_pass = $("#tushu_pass").val();
        if (tushu_user == "" || tushu_pass == "") {
            alert('不能有内容为空');
        } else {
            var login_pd = {'user': tushu_user, 'pwd': tushu_pass};
            $.ajax({
                type: "post",
                contentType: "application/json",
                url: "/operate/login",
