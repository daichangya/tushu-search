...
from tushu.fetcher.extract_novels import extract_chapters
from tushu.fetcher.function import get_time, get_netloc
from tushu.fetcher.decorators import authenticator, auth_params, response_handle
from tushu.fetcher import UniResponse, ResponseField
from tushu.fetcher.cache import cache_tushu_novels_chapter
from tushu.fetcher.novels_tools import get_novels_info
from tushu.config import LOGGER

api_bp = Blueprint('api_blueprint', url_prefix='api')


@api_bp.route("/owl_bd_novels/<name>")
@authenticator('Tushu_Api-Key')
...
async def owl_novels_chapters(request, **kwargs):
    ...
    try:
        res = await cache_tushu_novels_chapter(url=chapters_url, netloc=netloc)
        chapters_sorted = []
        if res:
